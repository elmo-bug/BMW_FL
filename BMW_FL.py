...
class Request_Set:
    def __init__(self,workers,requesters,num_per_type,budget):
        ...
        self.accuracy={'BMW_FL_g':[],'BMW_FL_s':[],'Conflict_Detect_seperate':[],'Conflict_Detect_overall':[],'RAFL_seperate':[],'RAFL_overall':[]}
        self.rep_per_round={'BMW_FL_g':[],'BMW_FL_s':[],'Conflict_Detect_seperate':[],'Conflict_Detect_overall':[],'RAFL_seperate':[],'RAFL_overall':[]}
...
#integrate the selction of participants in global model for convenience since the global can operate the local        
    def run(self,mode="get_rep",size_of_selection=10):
        ...
        if mode=='Conflict_Detect_overall':
            for requester in self.requesters:
                requester.init() 
            sum_pay=0
        ...
        if mode== "Conflict_Detect_seperate":
            for requester in self.requesters:
                requester.init() 
            each_pay=[0 for i in range(m)]
            sum_pay=0
        ...
        if mode=='RAFL_overall':
            sum_pay=0
            participants=[0 for i in range(n)]
            conf=[{} for i in range(m)]
            bid=[{'ID':i,"type":self.workers[i].type_ID,'bid':self.workers[i].bid,'q':self.workers[i].bid/self.rep[i]} for i in range(n)]
...
class Requester:
    def __init__(self,ID,budget,workers,num_per_type,num_requester,data,Lambda=0.2,beta1=0.5,beta2=4,delta_threshold=-0.01,omega=0.4,ksi=0.5,batch_size=100):
        ...
        self.results={'get_rep':[],'BMW_FL_g':[],'BMW_FL_s':[],'Conflict_Detect_seperate':[],'Conflict_Detect_overall':[],'RAFL_seperate':[],'RAFL_overall':[]}
...
class Worker:
    ...
    def __init__(self,input_size,hidden_size,output_size,accuracy,data,ID,type_ID,num_requesters,base_score=1,epochs=1,learning_rate=0.005,\
        range_of_bid={"low":4,"high":6},batch_size=100,):
